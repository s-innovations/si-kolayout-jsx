...
//type k = keyof KnockoutBindingHandlers;

export interface ValueKnockoutBindingHandlers<T> {
...
export interface HTMLInputElementValueBindings<T> {
    "value": T;
    valueUpdate?: "\"input\"" | "\"keyup\"" | "\"keypress\"" | "\"afterkeydown\"" | string
    valueAllowUnset?: string | boolean;
}
export interface HTMLInputElementTextInput {
    "textInput": string;
}
...
export function Binding<T>(value:keyof T){
    return value;
}

export type BindingNumber = HTMLInputElementValueBindings<number>;
export type BindingString = HTMLInputElementValueBindings<string>;

export type HTMLInputElementValueBindingsAttributes =
    { type: "password", "data-bind": BindingString | Array<BindingString> | string } & Partial<HTMLAttributes<HTMLInputElement>> |
    { type: "email", "data-bind": BindingString | Array<BindingString> | string } & Partial<HTMLAttributes<HTMLInputElement>> |
    { type: "text", "data-bind": BindingString | Array<BindingString> | string } & Partial<HTMLAttributes<HTMLInputElement>> |
    { type: "number", "data-bind": BindingNumber | Array<BindingNumber> | string } & Partial<HTMLAttributes<HTMLInputElement>>
...
export interface CompositionEvent<T> extends SyntheticEvent<T> {
    data: string;
}
...
export interface FocusEvent<T> extends SyntheticEvent<T> {
...
export interface KeyboardEvent<T> extends SyntheticEvent<T> {
    altKey: boolean;
    charCode: number;
    ctrlKey: boolean;
...
export interface TouchEvent<T> extends SyntheticEvent<T> {
    altKey: boolean;
...
export interface AnimationEvent<T> extends SyntheticEvent<T> {
    animationName: string;
    pseudoElement: string;
    elapsedTime: number;
}

export interface TransitionEvent<T> extends SyntheticEvent<T> {
    propertyName: string;
    pseudoElement: string;
    elapsedTime: number;
}
...
export interface UIEvent<T> extends SyntheticEvent<T> {
    detail: number;
    view: AbstractView;
}
...
declare global {
    ...
    namespace JSX {
        interface Element {
            childLayouts?: { [key: string]: JSXLayout<any> | KoLayout }
        }
...
function appendChild(parent: TestNode, child: undefined | string | TestNode | TestNode[] | JSXLayout<any>) {
    
    if (typeof child === "undefined") {

    } else if (Array.isArray(child)) {
        for (let childchild of child) {
            appendChild(parent, childchild);
        }
        ...
        if (child.childLayouts) {
            parent.childLayouts = parent.childLayouts || {};
            parent.childLayouts = Object.assign(child.childLayouts, parent.childLayouts);
            delete child.childLayouts;
        }
...
export function KnockoutJsxFactory(tagName: string | (new (...args: any[]) => JSXLayout<AttributeCollection>), attributes: AttributeCollection | null, ...children: any[]): Node | JSXLayout<any> | Node[] {
    ...
    if (tagName === "ko") {
        if ("layout" in attributes) {
            ...
            for (let child of children) {
                appendChild(div as TestNode, child);
            }
    ...
    if (attributes) {
        let koBinding = "";
        for (let key of Object.keys(attributes).filter(key => key.substr(0, 3) === "ko-")) {

            let test = key as keyof AttributeCollection;

            let binding = key.substr(3);
            koBinding += binding + ":" + attributes[key] + ", ";
            delete attributes[key];
        }
        if (koBinding.length > 0) {
            element.setAttribute("data-bind", koBinding.substr(0, koBinding.length - 2));
        }

        if ("data-bind" in attributes) {
            if (typeof attributes["data-bind"] === "object") {
                element.setAttribute("data-bind", Object.keys(attributes["data-bind"]).map(key => `${key}: ${attributes["data-bind"][key]}`).join(','));
    ...
    for (let child of children) {
        appendChild(element as TestNode, child);
    }
